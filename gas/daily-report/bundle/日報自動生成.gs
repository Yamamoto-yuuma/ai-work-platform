 ...
 *
 * ■ 繰り返しタスクの次回分は取れない
 *   Tasks API に繰り返し（recurrence）という概念が無い。繰り返しの管理は Google の
 *   画面側にあり、API から見えるのは実体になった回だけ。実際、画面に「9/15」と
 *   出ている毎週平日のタスクを API から引くと、返ってくるのは完了済みの 9/14 の回
 *   だけだった。つまり、定例業務をタスクで登録しても日報には出せない。
 *
 *   回避する方法は無いので、定例業務はカレンダーの繰り返し予定にするのが正しい。
 *   そちらは時刻を持つので AM / PM にも正しく入る。
 */
