  ...
  return (
    <div
      className={`rounded-xl ${base} ${
        /* 押せるカードは、触れたら 1px だけ手前へ出す。大きさは変えない */
        interactive ? "lift hover:border-brand/35 hover:shadow-lift" : ""
      } ${className}`}
    >
      {children}
    </div>
...
/*
  ...
  触れるとわずかに手前へ出て縁が濃くなり、押した瞬間に沈む（globals.css の .tap）。
  縁を変えるのは、面の色だけだと、並んだボタンのどれに当たっているかが
  分かりにくいため。動きは 0.16 秒で終える。
*/
const BTN_BASE =
  "tap inline-flex items-center justify-center gap-1.5 whitespace-nowrap rounded-[5px] border font-medium " +
  /*
...
const BTN_VARIANTS = {
  ...
  secondary:
    ...
    "hover:border-brand/55 hover:bg-brand-soft/50 hover:text-brand-ink hover:shadow-lift",
  ghost:
    "border-transparent bg-transparent text-ink-2 " +
    "hover:bg-surface-2 hover:text-ink",
  danger:
    "border-line bg-surface text-danger " +
    "hover:border-danger/55 hover:bg-danger-soft hover:shadow-lift",
} as const;

/* 大きさは画面の段（12 / 13.5 / 15px）に合わせる。ここだけ別の刻みにしない */
const BTN_SIZES = {
        ...
        return (
          ...
          >
            ...
            {it.count !== undefined && it.count > 0 && (
              <span className={`ml-1.5 rounded-full px-1.5 py-0.5 text-[12px] font-semibold ${
