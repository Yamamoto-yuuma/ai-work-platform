  ...
  return (
    ...
    >
        ...
        */}
        <div className={`rounded-xl border border-line-soft border-l-[3px] bg-surface p-6 shadow-card transition-shadow duration-150 hover:shadow-lift ${
          ...
          <div className="mb-2 flex items-center gap-2">
            <Eyebrow tone={next.urgency === "overdue" ? "danger" : "brand"}>First up</Eyebrow>
      ...
      */}
      ...
      {/*
        今日の予定（Schedule）。「今日いちばん先にやること」の次に「今日はこういう1日」を置く。
      ...
      */}
      <div className="grid gap-5 grid-cols-[minmax(0,1fr)] lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="flex flex-col gap-6">
          ...
          {dueChecks.length > 0 && (
            <Panel title="Needs review" count={dueChecks.length} note="確認予定日が来た業務">
              <RowList flat>
          ...
          {startable.length > 0 && (
            <Panel title="Ready to start" count={startable.length} note="開始条件が来た業務">
              <RowList flat>
                {startable.map((def) => (
                  <Row key={def.key} className="flex flex-wrap items-center gap-3 px-4 py-2.5">
                    <span className="min-w-0 flex-1">
                      <Link href={`/workflows/${def.key}`} className="block truncate text-[13.5px] font-medium hover:text-brand">
                        {def.name}
                      </Link>
                      {/*
          ...
          <Panel
            title="Up next"
            action={<Link href="/tasks" className="shrink-0 text-[13.5px] text-brand hover:underline">すべてのタスク</Link>}
          >
              ...
              <RowList flat>
                {upNext.map((a, i) => (
                  <Row key={i}>
                    ...
                    >
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-[13.5px] font-medium">{a.headline}</span>
                        <span className="mt-0.5 block text-[12px] text-ink-3">{a.reason}</span>
                      </span>
                      {a.dueAt && (
        ...
        <div className="flex flex-col gap-5">
          {proposed.length > 0 && (
            <Card className="bg-signal-soft p-4">
              <p className="text-[12px] font-semibold uppercase tracking-[0.11em] text-signal">Needs confirmation</p>
          ...
          {waiting.length > 0 && (
            <Panel title="Waiting" count={waiting.length}>
          ...
          */}
          {activeRuns.length > 0 && (
            <Panel
              title="In progress" count={activeRuns.length}
              action={<Link href="/workflows" className="shrink-0 text-[12px] text-brand hover:underline">すべての業務</Link>}
            >
              <ul className="flex flex-col gap-1.5 p-3">
                {activeRuns.map((run) => {
                  ...
                  return (
                    <li key={run.id}>
                      <Link href={`/navigator/${run.id}`} className="block rounded-lg bg-surface-2 px-3 py-2 hover:bg-brand-soft">
                        <span className="block truncate text-[13.5px] font-medium">{runLabel(run)}</span>
                        ...
                        {inCandidates && (
                          <span className="mt-0.5 block text-[12px] text-brand">本日の候補に表示中</span>
                        )}
                      </Link>
                    </li>
                  );
