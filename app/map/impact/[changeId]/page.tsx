...
import { Badge, Button, Card, Eyebrow, PageHeader, SubHead } from "@/ui/primitives";
...
export default function ImpactPage({ params }: { params: Promise<{ changeId: string }> }) {
  ...
  if (!change) return <div className="p-8 text-[13.5px]">変更イベントが見つかりません。</div>;
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-6 py-6">
      ...
      <Card className="mb-6 bg-brand-soft p-5">
        <Eyebrow tone="brand">The change</Eyebrow>
        <p className="mt-1.5 text-[19px] font-bold text-brand-ink">{change.entityLabel}</p>
        ...
        {change.reason && <p className="mt-2 text-[13.5px] text-ink-2">理由：{change.reason}</p>}
      </Card>

      {tasks.length === 0 ? (
        <Card className="p-8 text-center">
          <p className="text-[15px] font-medium">この変更の影響範囲を分析します</p>
        ...
        <>
                ...
                <Card key={layer.key} className="overflow-hidden">
                  <div className="border-b border-line bg-surface-2 px-4 py-2.5">
                    <div className="flex items-center justify-between">
                      <h3 className="text-[13.5px] font-bold">{layer.label}</h3>
                      <Badge tone={layer.tone}>{nodes.length}</Badge>
                    </div>
                    <p className="mt-0.5 text-[12px] text-ink-3">{layer.hint}</p>
                  </div>
                  <ul className="flex flex-col gap-1.5 p-3">
                    {nodes.length === 0 && <li className="py-2 text-center text-[12px] text-ink-3">該当なし</li>}
                    {nodes.map((n) => {
                      ...
                      return (
                        <li key={n.id} className={`rounded-lg border px-3 py-2.5 ${
                          ...
                          <p className="text-[13.5px] font-medium leading-snug">{n.label}</p>
                          {t?.description && <p className="mt-1 text-[12px] leading-relaxed text-ink-3">{t.description}</p>}
          ...
          {cycle && (
            <Card className="mb-6 bg-danger-soft p-4">
              <p className="text-[13.5px] font-bold text-danger">依存関係に循環があります</p>
              <p className="mt-1 text-[13.5px] text-danger">{cycle.join(" → ")}</p>
            </Card>
          )}
          ...
          {schedule && (
            <Card className="mb-6 p-5">
              <SubHead
                ...
                note={`逆算スケジュール（ゴール：${new Date(String(change.after)).toLocaleDateString("ja-JP")}）`}
                className="mb-0"
              />
              ...
              {schedule.warnings.length > 0 && (
                <ul className="mt-3 flex flex-col gap-1.5">
                  {schedule.warnings.map((w, i) => (
                    <li key={i} className="rounded-lg bg-danger-soft px-3 py-2 text-[13.5px] text-danger">⚠ {w.message}</li>
                  ))}
                </ul>
              )}
              ...
              <ol className="mt-4 flex flex-col gap-1">
                  ...
                  .map((p) => {
                    ...
                    return (
                      <li key={p.taskId} className="flex items-center gap-4 border-b border-line-soft py-2 last:border-b-0">
                        <span className="w-16 shrink-0 text-[13.5px] font-bold tabular-nums text-brand">
                ...
                <li className="flex items-center gap-4 pt-3">
                  <span className="w-16 shrink-0 text-[13.5px] font-bold tabular-nums text-ok">
                    {new Date(String(change.after)).toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" })}
                  </span>
                  <span className="flex-1 text-[13.5px] font-bold">ゴール：{change.entityLabel} {change.fieldLabel}</span>
                </li>
              </ol>
            </Card>
          )}
          ...
          {proposed.length > 0 && (
            <Card className="bg-signal-soft p-5">
              <p className="text-[15px] font-bold text-signal">{proposed.length}件のタスクが提案されています</p>
          ...
          {proposed.length === 0 && existing.length > 0 && (
            <Card className="bg-ok-soft p-4">
              <p className="text-[13.5px] font-bold text-ok">派生タスクは確定済みです</p>
              <Link href="/tasks?view=derived" className="mt-1 inline-block text-[13.5px] text-brand hover:underline">
                タスク一覧で確認する →
              </Link>
            </Card>
          )}
